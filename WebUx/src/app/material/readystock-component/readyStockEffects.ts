  ...
  export class ReadyStockEffects {   
    ...
    .map(action => {  
      ...
      .map(action => 
        ...
        @Effect() readyStockDelete$ = this.actions$
        .ofType(READYSTOCK_DELETE)
        .map(action => {    
          ...
          .map(action => {
            ...
            this.http.request("delete", APPLICATION_HOST + '/readystock/delete/', 
            { 
              headers : headersJson,
              body : action
            })
            .subscribe(res => {
              messageUtil.dispatchIntent(this.store, READYSTOCK_DELETE_SUCCESS, null);
              messageUtil.dispatchIntent(this.store, PROGRESS_WAIT_HIDE, null);
            ...
            err => {
              if (err && err.status == 204) {
                ...
                messageUtil.dispatchIntent(this.store, READYSTOCK_DELETE_ERR, null);
                messageUtil.dispatchIntent(this.store, PROGRESS_WAIT_HIDE, null);
              }            
            });
