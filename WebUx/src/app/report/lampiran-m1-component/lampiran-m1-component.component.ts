...
export class LampiranM1ComponentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    save() {
      ...
      this.dispatchIntent(M1LAMPIRAN_SAVE, mainFormModel);
      this.display = false;
    }
    ...
    componentMessageHandle(messageAll: Array<any>) {
      ...
      console.log('message all', messageAll);
      ...
      messageAll.map(async message => {        
        ...
        if (message && message.type == M1LAMPIRAN_GET_OK) {
          ...
          for (var lampiranItem of message.data.data.data) {
            var dataInfo = {...lampiranItem};
        ...
        { 
          ...
          if (message.data && message.data.data && message.data.data.data)
          {
            for (var configDataItem of message.data.data.data)
            {
              var configDataInfo = {...configDataItem};                  
    ...
    private configureAddForms() {
      ...
      this.dataForm = this.fb.group({
        ...
        'salesExpCont': ['', [Validators.required]],
        'salesGpb': ['', [Validators.required]],
        'salesFiz': ['', [Validators.required]],
        'salesLocal': ['', [Validators.required]], 
    ...
    private configureEditForm() {
      ...
      this.dataForm = this.fb.group({
        ...
        'salesExpCont': [this.data.salesExpCont, [Validators.required]],
        'salesGpb': [this.data.salesGpb, [Validators.required]],
        'salesFiz': [this.data.salesFiz, [Validators.required]],
        'salesLocal': [this.data.salesLocal, [Validators.required]], 
